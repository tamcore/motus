...
| `MOTUS_AI_SYSTEM_PROMPT` | _(built-in)_ | When non-empty, replaces the built-in system prompt entirely. The built-in prompt describes available tool categories, instructs the model to call `get_server_time` before resolving relative dates, and appends today's date. |
